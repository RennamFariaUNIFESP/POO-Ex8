...
removeUser(email: string): null  {
    const userIndex = this.users.findIndex(users => users.email === email)
    if (userIndex == -1) {
...
const user = this.findUser(userEmail)
    ...
    async authenticate(userEmail: string, password: string): Promise<boolean> {
        const user = this.findUser(userEmail)
        if(user === undefined)
            throw new UserNotFoundError()   //necessita?, desse segundo throw sendo que ja existe no findUser
